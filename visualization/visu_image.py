...
def save_camera_image(idx: int, camera_image: CameraImage,
    ...
    os.makedirs(str(output_dir / f'camera-ori-{CAMERA_NAME[camera_image.name]}'), exist_ok=True)
    name = f'camera-ori-{CAMERA_NAME[camera_image.name]}/camera-ori-{CAMERA_NAME[camera_image.name]}-{idx:03}.png'
    ...
    os.makedirs(str(output_dir / f'camera-label-{CAMERA_NAME[camera_image.name]}'), exist_ok=True)
    name = f'camera-label-{CAMERA_NAME[camera_image.name]}/camera-label-{CAMERA_NAME[camera_image.name]}-{idx:03}.png'
    cv2.imwrite(str(output_dir / name), image)


def draw_labels(image: np.ndarray, labels: CameraLabels) -> None:
    def _draw_label(label_: CameraLabels) -> None:
        def _draw_line(p1, p2) -> None:
...
def rgba_func(value: float):
    """Generates a color based on a range value"""
    return plt.get_cmap('jet')(value / 50.)
...
def plot_points_on_image(idx: int, projected_points: np.ndarray,
    ...
    os.makedirs(str(output_dir / f'range-image-{CAMERA_NAME[camera_image.name]}'), exist_ok=True)
    name = f'range-image-{CAMERA_NAME[camera_image.name]}/range-image-{CAMERA_NAME[camera_image.name]}-{idx:03}.png'
    cv2.imwrite(str(output_dir / name), image)
